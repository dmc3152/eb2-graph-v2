...
export const resumeTriviaGame: NonNullable<MutationResolvers['resumeTriviaGame']> = async (_parent, _arg, { dataSources }: RequestContext) => {
    ...
    if (!user.callings.map(x => x.id).includes('calling:executive_secretary')) {
        return {
            ...
            error: {
                code: "INSUFFICIENT_PERMISSIONS",
                message: "The authenticated user does not have permission to resume trivia games"
            }
        }
    }

    try {
        dataSources.triviaGame().resumeGame();
    }
    catch (error) {
        return {
            success: false,
