...
export const closeTriviaGame: NonNullable<MutationResolvers['closeTriviaGame']> = async (_parent, _arg, { dataSources }: RequestContext) => {
    ...
    if (!user.callings.map(x => x.id).includes('calling:executive_secretary')) {
        return {
            ...
            error: {
                code: "INSUFFICIENT_PERMISSIONS",
                message: "The authenticated user does not have permission to close trivia games"
            }
        }
    }

    try {
        dataSources.triviaGame().closeGame();
    }
    catch (error) {
        return {
            success: false,
