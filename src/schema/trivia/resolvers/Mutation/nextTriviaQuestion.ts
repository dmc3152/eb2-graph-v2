...
export const nextTriviaQuestion: NonNullable<MutationResolvers['nextTriviaQuestion']> = async (_parent, _arg, { dataSources }: RequestContext) => {
    ...
    if (!user.callings.map(x => x.id).includes('calling:executive_secretary')) {
        return {
            ...
            error: {
                code: "INSUFFICIENT_PERMISSIONS",
                message: "The authenticated user does not have permission to trigger the next question"
            }
        }
    }

    try {
        dataSources.triviaGame().nextQuestion();
    }
    catch (error) {
        return {
            success: false,
