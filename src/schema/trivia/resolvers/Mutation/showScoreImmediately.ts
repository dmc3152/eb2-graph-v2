...
export const showScoreImmediately: NonNullable<MutationResolvers['showScoreImmediately']> = async (_parent, _arg, { dataSources }: RequestContext) => {
    ...
    try {
        dataSources.triviaGame().showScoreImmediately();
    }
    catch (error) {
        return {
            success: false,
