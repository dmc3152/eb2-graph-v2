...
export const stopTriviaGame: NonNullable<MutationResolvers['stopTriviaGame']> = async (_parent, _arg, { dataSources }: RequestContext) => {
    ...
    if (!user.callings.map(x => x.id).includes('calling:executive_secretary')) {
        return {
            ...
            error: {
                code: "INSUFFICIENT_PERMISSIONS",
                message: "The authenticated user does not have permission to stop trivia games"
            }
        }
    }

    try {
        dataSources.triviaGame().stopGame();
    }
    catch (error) {
        return {
            success: false,
