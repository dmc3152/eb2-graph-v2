...
export const showScoreAfterQuestion: NonNullable<MutationResolvers['showScoreAfterQuestion']> = async (_parent, _arg, { dataSources }: RequestContext) => {
    ...
    if (!user.callings.map(x => x.id).includes('calling:executive_secretary')) {
        return {
            ...
            error: {
                code: "INSUFFICIENT_PERMISSIONS",
                message: "The authenticated user does not have permission to show scores"
            }
        }
    }

    try {
        dataSources.triviaGame().showScoreAfterQuestion();
    }
    catch (error) {
        return {
            success: false,
